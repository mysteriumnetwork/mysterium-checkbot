...
*Available commands:*
> \`!check <nodes ...>\` - Checks the availability of one or more Mysterium nodes
...
export const NODE_AVAILABLE_SUMMARY = ':white_check_mark: Node `%s` is available and has internet access as `%s`.'
...
export const NODE_AVAILABLE_MESSAGE = '<@%s>: :white_check_mark: Node `%s` is available and has internet access as `%s`.'
...
<@%s>: :scream: Sorry, an unexpected error occurred while processing your request. Please try again.
*Error:*
> Message: %s
`
